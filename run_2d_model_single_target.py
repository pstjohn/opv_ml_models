...
from nfp.models import masked_mean_squared_error, masked_mean_absolute_error
...
# Load the input data
train = pd.read_csv('data/smiles_train.csv.gz', index_col=0).sample(frac=1.)
valid = pd.read_csv('data/smiles_valid.csv.gz', index_col=0).sample(frac=1.)
...
# Rescale Y matrix
y_train_raw = train.set_index('smile')[[prop]].values
y_valid_raw = valid.set_index('smile')[[prop]].values
